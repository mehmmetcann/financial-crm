...
namespace FinancialCrm
{
    public partial class FrmDashboard : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (count % 4 == 0)
            {
                var internetFaturasi = db.Bills.Where(x => x.BillTitle == "İnternet Faturası" || x.BillTitle == "Internet Faturası").OrderByDescending(x => x.BillPeriod).Select(y => y.BillAmount).FirstOrDefault();

                lblBillTitle.Text = "İnternet Faturası";
                lblBillAmount.Text = (internetFaturasi ?? 0).ToString() + " ₺";
            }
        }
